# stage4 — 환경 기록

## 실험 메타

| 항목 | 값                                                     |
|---|-------------------------------------------------------|
| 실험 ID | `stage4`                                              |
| 실험 목적 | VU 1500 으로 상향, 모놀리틱 ×1 의 실제 한계 탐색|
| 날짜/시간 | `<2026-04-20 16:09 KST>`                              |
| 선행 실험 | [stage3](../stage3/) — CPU 1.0, VU 800, CPU 62.8%, 안정 |

## 아키텍처

```
[k6 container] ──▶ [nginx:80] ──▶ [app × 1 replica, cpus=1.0, mem=512M]
                                        │
                                        ├──▶ [mysql:8.0.38]
                                        └──▶ [redis:7.4]
```

- **레플리카 수**: 1 (모놀리틱 단일 인스턴스)
- **LB**: Nginx 경유
- **DB**: 컨테이너 MySQL, 볼륨 `mysql-data`
- **캐시**: 컨테이너 Redis (휘발성)

## 리소스 제한

| 서비스 | cpus | memory |
|---|---|---|
| app | **1.0** | 512M |
| mysql | (제한 없음) | (제한 없음) |
| redis | (제한 없음) | (제한 없음) |
| nginx | (제한 없음) | (제한 없음) |

## 이미지/버전

stage1 과 동일 — [stage1/env.md](../stage1/env.md) 참조.

## 부하 시나리오

- 스크립트: [`../../k6/board-load.js`](../../k6/board-load.js)
- 트래픽 믹스 근거: [`../../k6/METHODOLOGY.md §2~3`](../../k6/METHODOLOGY.md) (NN/g 90-9-1)
- stages: `1m → 200 VU` / `3m → 1500 VU` / `11m (hold 1500)` / `1m → 0 VU`
- 총 소요: 약 16분

## 호스트 사양

stage0 과 동일 — [stage0/env.md](../stage0/env.md) 참조.

## stage3 대비 변경점

| 항목 | stage3                                      | stage4                                       |
|---|---------------------------------------------|----------------------------------------------|
| **VU 상한** | 800                                         | **1500**                                     |
| stages 구성 | `1m→200 / 2m→800 / 11m hold / 1m→0` (총 15분) | `1m→200 / 3m→1500 / 11m hold / 1m→0` (총 16분) |
| app cpus | 1.0                                         | 1.0 (동일)                                     |
| app memory | 512M                                        | 512M (동일)                                    |
| 레플리카 | 1                                           | 1 (동일)                                       |
