...
export function browse() {
  ...
  if (lastArticleId) {
    ...
    // 0.065 ─ 1.000  read-only       (93.5%)
    // ※ create article (0.5%) 은 lastArticleId 불필요 — 빈 DB 데드락 방지 위해
    //   if (lastArticleId) 블록 밖으로 분리.
  ...
  // create article: lastArticleId 에 의존하지 않으므로 블록 밖에서 독립 실행.
  // 빈 DB 에서도 쓰기 경로가 동작해 시간이 지나면 자연스럽게 읽기 분기도 활성화됨.
